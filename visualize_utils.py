...
def visualizeBatchPair(options, config, inp_pair, detection_pair, indexOffset=0, prefix='', suffix='', write_ply=False,
                       write_new_view=False, writer=None):
    detection_images = []
    for pair_index, (input_dict, detection_dict) in enumerate(zip(inp_pair, detection_pair)):
        image_dict = visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=indexOffset,
                                             ...
                                             write_ply=write_ply, write_new_view=write_new_view, writer=writer)
...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='',
                            prediction_suffix='', write_ply=False, write_new_view=False, writer=None):
    image_dict = {}
    ...
    if 'warped_image' in input_dict:
        ...
        cv2.imwrite(options.test_dir + '/' + ('%04d' % indexOffset) + '_image' + suffix + '_warped.png',
                    warped_image[80:560])
        pass

    if 'warped_depth' in input_dict:
        warped_depth = input_dict['warped_depth'].detach().cpu().numpy()
        cv2.imwrite(options.test_dir + '/' + ('%04d' % indexOffset) + '_depth' + suffix + '_warped.png',
    ...
    if 'mask' in input_dict:
        ...
        if config.NUM_PARAMETER_CHANNELS > 0 and not config.OCCLUSION:
            ...
            for box, patch_depth in zip(boxes, depths):
                ...
                continue
            cv2.imwrite(options.test_dir + '/' + ('%04d' % indexOffset) + '_depth_patch' + suffix + '.png',
    ...
    if 'boundary' in detection_dict:
        ...
        for name, boundary in [('gt', boundary_gt), ('pred', boundary_pred)]:
            ...
            cv2.imwrite(options.test_dir + '/' + ('%04d' % indexOffset) + '_boundary' + suffix + '_' + name + '.png',
                        boundary_image)
            continue
        pass
    ...
    if 'depth' in detection_dict:
        ...
        cv2.imwrite(options.test_dir + '/' + ('%04d' % indexOffset) + '_depth' + suffix + prediction_suffix + '.png',
    ...
    if 'depth_ori' in detection_dict:
        cv2.imwrite(
            options.test_dir + '/' + ('%04d' % indexOffset) + '_depth' + suffix + prediction_suffix + '_ori.png',
    ...
    if write_new_view and False:
        ...
        drawNewViewDepth(
            options.test_dir + '/' + ('%04d' % indexOffset) + '_new_view' + suffix + prediction_suffix + '.png',
        ...
        drawNewViewDepth(options.test_dir + '/' + (
                    '%04d' % indexOffset) + '_new_view_depth_gt' + suffix + prediction_suffix + '.png',
                         np.expand_dims(depth > 1e-4, 0), np.expand_dims(XYZ_gt, 0),
                         input_dict['camera'].detach().cpu().numpy(), pose)
        depth = detection_dict['depth_np'].squeeze()[80:560]
        ranges = config.getRanges(input_dict['camera']).detach().cpu().numpy()
        XYZ_gt = ranges * np.expand_dims(depth, axis=-1)
        drawNewViewDepth(options.test_dir + '/' + (
                    '%04d' % indexOffset) + '_new_view_depth_pred' + suffix + prediction_suffix + '.png',
                         np.expand_dims(depth > 1e-4, 0), np.expand_dims(XYZ_gt, 0),
                         input_dict['camera'].detach().cpu().numpy(), pose)
        pass
    ...
    if write_new_view:
        ...
        for c in range(len(locations) - 1):
            ...
            for frame in range(num_frames[c]):
                ...
                drawNewViewDepth(
                    options.test_dir + '/' + ('%04d' % indexOffset) + '_video/' + str(index_offset) + '.png',
                    detection_masks, XYZ_pred, camera, pose)

                drawNewViewDepth(
                    options.test_dir + '/' + ('%04d' % indexOffset) + '_video_gt/' + str(index_offset) + '.png',
    ...
    if write_ply:
        ...
        writePLYFileMask(
            options.test_dir + '/' + ('%04d' % indexOffset) + '_model' + suffix + prediction_suffix + '.ply',
        ...
        with open(pose_filename, 'w') as f:
            ...
            f.close()
            pass
        model_filename = current_dir + '/' + options.test_dir + '/' + (
                    '%04d' % indexOffset) + '_model' + suffix + prediction_suffix + '.ply'
        output_filename = current_dir + '/' + options.test_dir + '/' + (
                    '%04d' % indexOffset) + '_model' + suffix + prediction_suffix + '.png'
        try:
            os.system(
