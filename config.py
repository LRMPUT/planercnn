...
class Config(object):
    ...
    def __init__(self, options, use_gpu=True):
        ...
        if use_gpu:
            ...
            if hasattr(self, 'ANCHOR_NORMALS_TENSOR'):
                self.ANCHOR_NORMALS_TENSOR = self.ANCHOR_NORMALS_TENSOR.cuda()
            if hasattr(self, 'ANCHOR_OFFSETS_TENSOR'):
                self.ANCHOR_OFFSETS_TENSOR = self.ANCHOR_OFFSETS_TENSOR.cuda()
            if hasattr(self, 'ANCHOR_NORMALS_TENSOR'):
                self.ANCHOR_NORMALS_TENSOR = self.ANCHOR_NORMALS_TENSOR.cuda()
            if hasattr(self, 'ANCHOR_PLANES_TENSOR'):
                self.ANCHOR_PLANES_TENSOR = self.ANCHOR_PLANES_TENSOR.cuda()
    ...
    def loadAnchorPlanes(self, anchor_type = ''):
        ...
        with torch.no_grad():
            if self.ANCHOR_TYPE == 'none':
                ...
                self.ANCHOR_PLANES_TENSOR = torch.from_numpy(self.ANCHOR_PLANES.astype(np.float32))
                ...
                self.ANCHOR_OFFSETS_TENSOR = torch.from_numpy(self.ANCHOR_OFFSETS.astype(np.float32))
    ...
    def getRanges(self, metadata):
        urange = (self.URANGE_UNIT * metadata[4] - metadata[2]) / metadata[0]
        vrange = (self.VRANGE_UNIT * metadata[5] - metadata[3]) / metadata[1]
    ...
    def getRangesFull(self, metadata):
        ...
        cy_diff = (self.IMAGE_MAX_DIM - metadata[5]) / 2
        urange = (self.URANGE_UNIT_F * self.IMAGE_MAX_DIM - metadata[2]) / metadata[0]
        vrange = (self.VRANGE_UNIT_F * self.IMAGE_MAX_DIM - metadata[3] - cy_diff) / metadata[1]
